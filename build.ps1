...
try {
    python -m PyInstaller --noconfirm --clean --onefile --windowed `
        ...
        (Join-Path $PSScriptRoot "overlay_board.py")
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $outputPath)) {
        throw "Overlay Board build failed with exit code $LASTEXITCODE."
    }
}
finally {
